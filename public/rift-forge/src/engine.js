...
const RF = (() => {
  ...
  const BOSSES={
    ...
    reaper:{name:'凜獄斬刑者',english:'THE RIME EXECUTIONER',subtitle:'避開雙重斬線，閃避可解除寒霜緩速',sprite:'reaper',color:'#93ddeb',hp:2300,r:50,wave:3},
    queen:{name:'霜骸女王',english:'THE FROSTBONE QUEEN',subtitle:'尋找暴風雪空隙，優先清除召喚獵犬',sprite:'queen',color:'#b5b5ff',hp:3900,r:48,wave:7}
  };
  const CHAPTERS={
    1:{name:'沉沒鑄造所',english:'THE SUNKEN FOUNDRY',waves:8,duration:24,final:'bell',mid:{3:'maw',6:'oracle'},shops:[4,7],events:[3,6,8],mobs:['guard','runner','seer'],bosses:3},
    2:{name:'霜骸王庭',english:'THE FROSTBONE COURT',waves:6,duration:28,final:'queen',mid:{3:'reaper'},shops:[4],events:[2,5,6],mobs:['iceguard','icehound','icewitch'],bosses:2}
  };
  const chapter=s=>CHAPTERS[s.chapter||1]||CHAPTERS[1];
  const ENCOUNTERS={
  ...
  function metaSafe(m={}){const num=(x,max)=>Number.isFinite(x)?clamp(Math.floor(x),0,max):0;return {v:1,credits:num(m.credits,1e7),forge:num(m.forge,5),ward:num(m.ward,5),archive:num(m.archive,3),runs:num(m.runs,1e6),wins:num(m.wins,1e6),best:num(m.best,1e7),chapter2Wins:num(m.chapter2Wins,1e6)};}
  function unlocked(meta,id){return id===1||id===2&&metaSafe(meta).wins>0;}
  function create(meta={},seed=Date.now(),chapterId=1){
    meta=metaSafe(meta);
    const s={v:1,chapter:1,chill:0,rng:seed>>>0,phase:'play',time:0,wave:1,waveTime:0,nextSpawn:0.4,spawned:0,bossSpawned:false,
    ...
    if(chapterId===2&&unlocked(meta,2)){
      s.chapter=2;s.level=10;for(let i=1;i<10;i++)s.xpNeed=Math.round(s.xpNeed*1.18+2);
      s.maxHp+=30;s.hp=s.maxHp;s.charge=100;s.rerolls+=2;s.passives.power=1;s.passives.haste=1;
      s.loadout=['ember','frost','spark'].map(id=>({id,level:2}));
    }
    for(const x of [350,480,610])spawn(s,chapter(s).mobs[0],x,x===480?116:134);s.eventOrder=shuffle(s,Object.keys(ENCOUNTERS));return s;
  }
  function nextChapter(s){
    if(s.phase!=='win'||s.chapter!==1||!s.paid||!s.bossesDefeated.includes('bell'))return false;
    Object.assign(s,{chapter:2,phase:'play',wave:1,waveTime:0,time:0,nextSpawn:1.4,bossSpawned:false,encounter:null,bossesDefeated:[],hazards:[],enemies:[],balls:[],bullets:[],fx:[],ghosts:[],texts:[],events:[],offers:[],eventIndex:0,encounterEvent:null,eventHistory:[],pact:null,chill:0,telegraph:0,coins:0,kills:0,catches:0,damage:{},peakBalls:0,peakEnemies:0,paid:false,earned:undefined,shotCd:.5,fireAnim:0,dashCd:0,invuln:1,x:480});
    s.hp=s.maxHp;s.charge=100;s.rerolls++;s.eventOrder=shuffle(s,Object.keys(ENCOUNTERS));
    for(const x of [350,480,610])spawn(s,'iceguard',x,134);event(s,'chapter');return true;
  }
  function spawn(s,kind,x,y,bossId='bell'){if(s.enemies.length>=70)return;
    const stats={guard:[26,29,8],runner:[19,19,16],seer:[23,23,7],iceguard:[28,85,8],icehound:[21,62,14],icewitch:[24,76,6],boss:[54,2000,0]}[kind];
    ...
    const hp=boss?boss.hp:stats[1]*(1+(s.wave-1)*(s.chapter===2?.16:.25));
    s.enemies.push({id:++s.uid,kind,...(boss?{bossId,attack:0,summon:8}:{}),...(kind==='iceguard'?{guard:3}:{}),...(kind==='icehound'?{pounce:3.4,windup:0,rush:0,targetX:x}:{}),x,y,r:boss?boss.r:stats[0],hp,maxHp:hp,speed:stats[2],slow:0,burn:0,poison:0,burnD:0,poisonD:0,burnOwner:'ember',poisonOwner:'thorn',flash:0,cast:0,age:0,shot:boss?2.2:2+rand(s)*2,seed:rand(s)*6.28,dead:false});
  }
  ...
  function hit(s,e,d,owner,secondary=false){if(e.dead||d<=0)return;
    ...
    if(e.hp<=0){e.dead=true;if(s.ghosts.length>=28)s.ghosts.shift();s.ghosts.push({kind:e.kind,bossId:e.bossId,x:e.x,y:e.y,r:e.r,life:.55,max:.55,seed:e.seed});s.kills++;s.xp+=e.kind==='boss'?(s.chapter===2?65:30):(s.chapter===2?6:3);s.coins+=e.kind==='boss'?35:2;s.charge=clamp(s.charge+3,0,100);fx(s,'death',e.x,e.y,TYPES[owner]?.color||'#edc889',e.r/12);event(s,'kill');if(e.kind==='boss')defeatBoss(s,e.bossId||'bell');}
  }
  ...
  function advanceWave(s){s.wave++;s.waveTime=0;s.nextSpawn=1.6;s.pact=null;event(s,'wave');
    const c=chapter(s);if(s.wave===c.waves+1){beginBoss(s,c.final);return;}
    if(c.shops.includes(s.wave)){s.phase='shop';makeOffers(s);}
    else if(c.events.includes(s.wave))openEncounter(s);
  }
  function defeatBoss(s,id){if(!s.bossesDefeated.includes(id))s.bossesDefeated.push(id);s.encounter=null;s.bullets=[];s.hazards=[];s.telegraph=0;
    if(id===chapter(s).final){s.phase='win';s.offers=[];event(s,'win');return;}
  ...
  function hazard(s,x,width,kind,delay=1.25,damage=kind==='star'?17:20){if(s.hazards.length>=12)return;s.hazards.push({x:clamp(x,LEFT+width/2,RIGHT-width/2),width,kind,life:delay,max:delay,damage});}
  ...
  function bossAttack(s,e){const id=e.bossId||'bell',rage=e.hp<e.maxHp*.5;e.attack=(e.attack||0)+1;e.cast=.35;
    if(id==='maw'){const n=rage?7:5;for(let i=0;i<n;i++)enemyBall(s,e,Math.PI*.22+i/(n-1)*Math.PI*.56,rage?154:128,'maw');hazard(s,s.x,rage?108:86,'fissure',1.3);e.shot=rage?2.5:3.2;}
      ...
      const target=clamp(s.x,LEFT+65,RIGHT-65);hazard(s,target,94,'ice',1.4,24);
      hazard(s,target<480?target+160:target-160,94,'ice',1.7,24);
      const n=rage?7:5;for(let i=0;i<n;i++)enemyBall(s,e,Math.PI*.22+i/(n-1)*Math.PI*.56,rage?166:138,'ice');e.shot=rage?2.7:3.5;
    }
    else if(id==='queen'){
      // Four lanes always leave at least one 184px corridor completely safe.
      const safe=(e.attack-1)%4;for(let i=0;i<4;i++)if(i!==safe)hazard(s,LEFT+92+i*184,rage?118:98,'blizzard',1.5+(i%2)*.2,22);
      const aim=Math.atan2(520-e.y,s.x-e.x);for(let i=-1;i<=1;i++)enemyBall(s,e,aim+i*.28,rage?178:148,'ice');e.shot=rage?3:3.8;
    }
  ...
  function impact(s,b,e){
    const id=b.type,lv=b.level,kind=TYPES[id];let d=b.power*(1+s.passives.bank*.18*Math.min(3,b.banks));
    if(e.kind==='iceguard'&&e.guard>0){if(b.banks>0){e.guard=0;fx(s,'shatter',e.x,e.y,'#a9edff',1.5);}else{e.guard--;d*=.45;if(e.guard===0)fx(s,'shatter',e.x,e.y,'#a9edff',1.5);}}
  ...
  function damagePlayer(s,d){if(s.phase!=='play'||s.invuln>0)return false;s.hp=Math.max(0,s.hp-d*(s.pact?.wave===s.wave?s.pact.incoming:1));s.invuln=.55;fx(s,'hurt',s.x,520,'#f37878',2);event(s,'hurt');if(s.hp<=0){s.phase='lose';s.offers=[];event(s,'lose');}return true;}
  ...
  function dash(s,dir=1){if(s.phase!=='play'||s.dashCd>0)return false;const from=s.x;s.x=clamp(s.x+dir*125,LEFT+22,RIGHT-22);s.chill=0;fx(s,'dash',from,520,'#a3dfeb',1,s.x,520);s.invuln=.65;s.dashCd=3.5;fx(s,'ring',s.x,520,'#a3dfeb',2);event(s,'dash');return true;}
  function step(s,dt,input={}){
    ...
    s.chill=Math.max(0,(s.chill||0)-dt);s.x=clamp(s.x+(input.move||0)*330*(s.chill>0?.7:1)*dt,LEFT+22,RIGHT-22);
    ...
    const c=chapter(s);
    if(s.wave<=c.waves&&!s.encounter){s.nextSpawn-=dt;if(s.nextSpawn<=0){const count=s.wave<3?2:3;for(let j=0;j<count;j++){const k=rand(s);spawn(s,c.mobs[k<(s.chapter===2?.46:.52)?0:k<(s.chapter===2?.76:.79)?1:2],LEFT+40+rand(s)*(RIGHT-LEFT-80),TOP+27);}s.nextSpawn=Math.max(1.7,(s.chapter===2?3.5:4)-s.wave*.2);}}
    if(s.waveTime>=c.duration&&s.wave<=c.waves&&!s.encounter){const id=c.mid[s.wave];if(id&&!s.bossesDefeated.includes(id))beginBoss(s,id);else advanceWave(s);if(s.phase!=='play')return;}
    for(const e of s.enemies){if(e.dead)continue;e.age+=dt;e.flash=Math.max(0,e.flash-dt);e.cast=Math.max(0,(e.cast||0)-dt);e.slow=Math.max(0,e.slow-dt);
      ...
      let speed=e.speed;
      if(e.kind==='icehound'){
        if(e.windup>0){e.windup=Math.max(0,e.windup-dt);speed=0;if(e.windup===0){e.rush=1.15;e.cast=.3;}}
        else if(e.rush>0){e.rush=Math.max(0,e.rush-dt);speed*=3.3;e.x=clamp(e.x+clamp(e.targetX-e.x,-95,95)*dt,LEFT+e.r,RIGHT-e.r);}
        else{e.pounce-=dt;if(e.pounce<=0){e.pounce=4;e.windup=.65;e.targetX=s.x;speed=0;}}
      }
      e.y+=speed*(e.slow>0?.55:1)*dt;
      ...
      if(e.kind==='boss'&&e.bossId==='queen'){e.summon-=dt;if(e.summon<=0){if(s.enemies.filter(q=>!q.dead&&q.kind!=='boss').length<6)for(const x of [e.x-95,e.x+95])spawn(s,'icehound',clamp(x,LEFT+28,RIGHT-28),TOP+45);e.summon=e.hp<e.maxHp*.5?9:12;}}
      if(['seer','icewitch','boss'].includes(e.kind)){
        e.shot-=dt;if(e.shot<=0){if(e.kind==='boss')bossAttack(s,e);else if(e.kind==='icewitch'){e.cast=.4;const aim=Math.atan2(520-e.y,s.x-e.x);for(const offset of [-.18,.18])enemyBall(s,e,aim+offset,138,'ice');hazard(s,s.x,62,'ice',1.6,14);e.shot=6.2;}else{e.cast=.28;enemyBall(s,e,Math.atan2(520-e.y,s.x-e.x),150,'seer');e.shot=4;}}
      }
      if(e.y+e.r>=FLOOR){e.dead=true;damagePlayer(s,e.kind==='iceguard'?18:e.kind==='icehound'?15:e.kind==='guard'?14:e.kind==='runner'?10:12);}
    }
    if(s.phase!=='play')return;
    for(const h of s.hazards){h.life-=dt;if(h.life<=0){const cold=['ice','blizzard'].includes(h.kind);fx(s,'hazard',h.x,520,cold?'#a9edff':h.kind==='star'?'#c5aaff':'#ff9b64',h.width,0,0,h.kind);if(Math.abs(s.x-h.x)<h.width/2+12&&damagePlayer(s,h.damage)&&cold)s.chill=1.2;}}s.hazards=s.hazards.filter(h=>h.life>0);
    ...
    for(const b of s.bullets){b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;if(Math.hypot(b.x-s.x,b.y-520)<18+b.r){if(damagePlayer(s,b.kind==='ice'?10:8)&&b.kind==='ice')s.chill=1.2;b.life=0;}if(b.x<LEFT-30||b.x>RIGHT+30||b.y>570)b.life=0;}
  ...
  function settle(s,m){m=metaSafe(m);if(!['win','lose'].includes(s.phase)||s.paid)return m;s.paid=true;const reward=Math.floor(s.coins*.7)+s.wave*5+(s.phase==='win'?(s.chapter===2?140:80):0);m.credits+=reward;m.runs++;if(s.phase==='win'){m.wins++;if(s.chapter===2)m.chapter2Wins++;}m.best=Math.max(m.best,s.kills);return m;}
  ...
  function restore(raw){try{
    ...
    if(raw.chapter!==undefined&&![1,2].includes(raw.chapter))return null;
    ...
    if(raw.wave>chapter(raw).waves+1)return null;
    if(raw.enemies.some(e=>!['guard','runner','seer','iceguard','icehound','icewitch','boss'].includes(e.kind)||e.bossId&&!BOSSES[e.bossId]))return null;
    if(raw.encounter&&!BOSSES[raw.encounter]||raw.phase==='event'&&!ENCOUNTERS[raw.encounterEvent])return null;
    if(raw.hazards&&(!Array.isArray(raw.hazards)||raw.hazards.length>12||raw.hazards.some(h=>!Number.isFinite(h.x)||!Number.isFinite(h.width)||h.width<1||h.width>200||!Number.isFinite(h.life)||h.life<0||h.life>5||!Number.isFinite(h.max)||h.max<=0||!Number.isFinite(h.damage)||!['star','fissure','bell','ice','blizzard'].includes(h.kind))))return null;
    ...
    s.chill=Number.isFinite(s.chill)?clamp(s.chill,0,1.2):0;
  ...
  return {W,H,LEFT,RIGHT,TOP,FLOOR,TYPES,RECIPES,HERO_FORMS,heroForm,spellArt,PASSIVES,BOSSES,CHAPTERS,chapter,unlocked,nextChapter,ENCOUNTERS,clamp,rand,create,spawn,step,hit,impact,ball,ready,evolve,makeOffers,applyOffer,reroll,pulse,dash,settle,buy,cost,metaSafe,snapshot,restore,beginBoss,openEncounter,eventChoices,chooseEncounter};
})();
