...
const RF = (() => {
  ...
  function spawn(s,kind,x,y){if(s.enemies.length>=70)return;
    ...
    s.enemies.push({id:++s.uid,kind,x,y,r:stats[0],hp,maxHp:hp,speed:stats[2],slow:0,burn:0,poison:0,burnD:0,poisonD:0,burnOwner:'ember',poisonOwner:'thorn',flash:0,cast:0,age:0,shot:2+rand(s)*2,seed:rand(s)*6.28,dead:false});
  }
  function fx(s,type,x,y,color,size=1,x2=0,y2=0,core=''){if(s.fx.length>=210)return;const life=type==='pulse'?.65:type==='death'?.6:type==='muzzle'?.15:type==='wall'?.22:.4;s.fx.push({type,x,y,color,size,x2,y2,core,life,max:life});}
  ...
  function hit(s,e,d,owner,secondary=false){if(e.dead||d<=0)return;
    ...
    if(e.hp<=0){e.dead=true;if(s.ghosts.length>=28)s.ghosts.shift();s.ghosts.push({kind:e.kind,x:e.x,y:e.y,r:e.r,life:.55,max:.55,seed:e.seed});s.kills++;s.xp+=e.kind==='boss'?30:3;s.coins+=e.kind==='boss'?55:2;s.charge=clamp(s.charge+3,0,100);fx(s,'death',e.x,e.y,TYPES[owner]?.color||'#edc889',e.r/12);event(s,'kill');if(e.kind==='boss'){s.phase='win';s.offers=[];event(s,'win');}}
  }
  ...
  function step(s,dt,input={}){
    if(s.phase!=='play')return;dt=clamp(dt,0,1/30);s.time+=dt;s.waveTime+=dt;s.shotCd-=dt;s.invuln=Math.max(0,s.invuln-dt);s.dashCd=Math.max(0,s.dashCd-dt);s.fireAnim=Math.max(0,(s.fireAnim||0)-dt);s.moveDir=input.move||0;for(const g of s.ghosts)g.life-=dt;s.ghosts=s.ghosts.filter(g=>g.life>0);
    ...
    for(const e of s.enemies){if(e.dead)continue;e.age+=dt;e.flash=Math.max(0,e.flash-dt);e.cast=Math.max(0,(e.cast||0)-dt);e.slow=Math.max(0,e.slow-dt);
      ...
      if(e.kind==='seer'||e.kind==='boss'){
        e.shot-=dt;if(e.shot<=0){e.cast=.28;const boss=e.kind==='boss',rage=e.hp<e.maxHp*.5;
          const num=boss?(rage?9:5):1;for(let n=0;n<num;n++){if(s.bullets.length>=90)break;const a=boss?Math.PI*.18+n/(num-1)*Math.PI*.64:Math.atan2(520-e.y,s.x-e.x);s.bullets.push({x:e.x,y:e.y+15,vx:Math.cos(a)*(boss?140:150),vy:Math.sin(a)*(boss?140:150),r:boss?6:5,life:8,kind:boss?'boss':'seer'});}
