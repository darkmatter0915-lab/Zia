...
0.6 攻擊特效、三場 Boss 與隨機事件更新，2026-09-25。依公開玩法重新設計的獨立單人反彈 Roguelite，不是 BALL x PIT 原始碼或素材的復原。
...
1 名角色、1 個原創 Canvas 戰鬥場景、3 種小怪、3 個雙階段頭目、8 波敵襲、5 種事件池（單局 3 次不重複）。6 種基本球核及 3 組指定進化；四格配装；三選一升級；有限重抽；五種刻印；兩個波次間商店；結算傷害統計；三個有限等級的基地升級。
...
新增熔獄巨顎及裂星司祭圖集，三場 Boss 分別鎖定第 3、6 波末與最終戰。五種隨機事件可提供回血、碎晶、重抽、刻印及有代價的強化；選擇存檔不會因重新載入而重抽。九種彈核的軌跡、碎片、電弧、回流與進化爆發重新細化。詳見 `../../docs/rift-forge-v06.md`，完整生成提示見 `../../docs/rift-forge-v06-art-prompts.json`。

## 0.7 元素戰鬥美術

火球、分岔閃電、冰槍與覆霜冰殼直接套用於實際戰鬥；九種彈核各有飛行與命中特效，完整及精簡模式都保留元素形態。特效改由 `src/elements.js` 原生 Canvas 繪製，沿用既有角色與場景。詳見 `../../docs/rift-forge-v07.md`。
