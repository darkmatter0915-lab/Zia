...
- [ ] 每個非 `Idle` 必要 Clip 均讓至少 1% 以非退化可見三角形表面積加權的人體蒙皮，在完整父子階層、inverse-bind matrices 與每頂點 influence 權重合成後，仍產生至少 1° 的實際蒙皮方向變化或 1 cm 的實際蒙皮頂點世界位移；不是空 Key、微小抖動、極低表面覆蓋、父子曲線或多骨權重互相抵銷的假動畫。
